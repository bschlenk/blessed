...
export default class Program extends EventEmitter {
  ...
  static instances = [];
  static _bound = false;
  static _exitHandler: (...args: any[]) => void = () => {};

  public type = 'program';

  // TODO: figure out how to type that this is either options or two params
  constructor(
    ...
    // TODO: Figure out if these are on the definition
  ...
  setupDump() {
    ...
    this.output.write = (data) => {
      this._log('OUT', stringify(data));
      return write.apply(this, arguments);
    };
  }

  setupTput() {
    // TODO: make a callonce annotation
    if (this._tputSetup) {
      return;
    }
    ...
    Object.keys(tput).forEach((key) => {
      ...
      if (tput.padding) {
        ...
        this.put[key] = () => {
          return this._write(tput[key].apply(tput, arguments));
        };
      }
    });
  };
  ...
  // deprecate this in favor of terminal = '''
  ...
  _listenInput() {
    ...
    this.input.on('keypress', this.input._keypressHandler = (ch, key) => {
      ...
      if (key.name === 'return' && key.sequence === '\r') {
        self.input.emit('keypress', ch, Object.assign({}, key, { name: 'enter' }));
      }
  ...
  removeKey(key, listener) {
    ...
    key.forEach((key) => {
      this.removeListener('key ' + key, listener);
    });
  }
  ...
  _buffer(text) {
    ...
    // TODO: void returns and bool returns?
    return true;
  }
  ...
  // TODO: this doesn't use any members, move to utils
  repeat(ch, i) {
    if (!i || i < 0) {
      i = 0;
    }
...
Program.prototype.setLocatorEvents = function() {
  ...
  pause(callback) {
    ...
    // TODO: move this into resume and add an isPaused flag
    return this._resume = () => {
      delete this._resume;
